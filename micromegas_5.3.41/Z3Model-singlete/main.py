...
def guardar_datos(X,SIGMA,OT): 
	n = int(len(SIGMA))
	archivo = open("datosSigma2.txt",'w')
	for i in range(n): 
		for valor in X[i]: 
...
if __name__ == '__main__': 
	...
	dat = np.loadtxt("AlmacenarDatos/datosLikelihood-mass1000-10000.txt")
	print("El tamaño de los datos es:"+str(len(dat)))

	x = dat.T[0]
	y = dat.T[1]
	z = dat.T[2]
	n = len(x)
	print("El tamaño de los datos es:"+str(n))
